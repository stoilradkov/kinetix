...
import {
    ...
    type TrainingProfileMutationMetadata,
...
@ApiTags("training profile")
@Controller({ path: "training/profile", version: "1" })
export class TrainingProfileController {
    constructor(
        @Inject(TRAINING_PROFILE_COMMANDS)
        private readonly commands: TrainingProfileCommands,
        @Inject(TRAINING_PROFILE_REPOSITORY)
        private readonly repository: TrainingProfileRepository,
        @Optional()
    ...
    @Get()
    @ApiOperation({ summary: "Get the active training profile" })
    async get(@Res({ passthrough: true }) response: HeaderResponse): Promise<TrainingProfileResponse> {
        const resource = await this.repository.readActive();
        if (!resource) throw new ApplicationNotFoundError("No active training profile exists");
        response.setHeader("ETag", formatRevisionEtag(resource.version));
        return trainingProfileResponseSchema.parse(resource);
    }

    @Post()
    @ApiOperation({ summary: "Create the training profile for the active core profile" })
    ...
    create(
        ...
        const request = parseContract(
            createTrainingProfileRequestSchema,
            rawBody,
            "Training profile creation validation failed",
        );
        const metadata = mutationMetadata(rawCorrelationId);
        return this.executeMutation({
            operation: "training.profile.create",
            idempotencyKey,
            request,
            metadata,
            response,
    ...
    @Patch()
    @ApiOperation({ summary: "Update the active training profile" })
    ...
    update(
        ...
        const request = parseContract(
            updateTrainingProfileRequestSchema,
            rawBody,
            "Training profile update validation failed",
        );
        const expectedVersion = expectedVersionFrom(ifMatch);
        const metadata = mutationMetadata(rawCorrelationId);
        return this.executeMutation({
            operation: "training.profile.update",
            idempotencyKey,
    ...
    private async executeMutation(input: {
        ...
        readonly metadata: TrainingProfileMutationMetadata;
        readonly response: HeaderResponse;
        readonly status: number;
        readonly command: (transaction?: unknown) => Promise<TrainingProfileResource>;
    }): Promise<TrainingProfileResponse> {
        const perform = async (transaction?: unknown) =>
            trainingProfileResponseSchema.parse(await input.command(transaction));
        let body: TrainingProfileResponse;
...
function mutationMetadata(
    ...
    reason?: string | null,
): TrainingProfileMutationMetadata {
    return {
