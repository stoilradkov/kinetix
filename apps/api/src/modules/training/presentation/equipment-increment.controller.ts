import { randomUUID } from "node:crypto";

import {
    Body,
    Controller,
    Get,
    Headers,
    HttpException,
    Inject,
    Optional,
    Param,
    Patch,
    Post,
    Query,
    Res,
} from "@nestjs/common";
import { ApiHeader, ApiOperation, ApiParam, ApiQuery, ApiTags } from "@nestjs/swagger";

import {
    createEquipmentIncrementRequestSchema,
    equipmentIncrementListResponseSchema,
    equipmentIncrementResponseSchema,
    updateEquipmentIncrementRequestSchema,
    type EquipmentIncrementListResponse,
    type EquipmentIncrementResponse,
} from "@kinetix/types";

import {
    EQUIPMENT_INCREMENT_COMMANDS,
    EQUIPMENT_INCREMENT_QUERIES,
    EquipmentIncrementNotFoundError,
    type EquipmentIncrementCommands,
    type EquipmentIncrementMutationMetadata,
    type EquipmentIncrementQueries,
    type EquipmentIncrementResource,
} from "#src/modules/training/application/index";
import {
    ApplicationValidationError,
    ExpectedVersionRequiredError,
    IDEMPOTENT_COMMAND_EXECUTOR,
    type IdempotentCommandExecutor,
} from "#src/platform/application/index";
import { entityId } from "#src/platform/domain/index";
import { formatRevisionEtag, parseRevisionEtag } from "#src/platform/presentation/revision-etag";

interface HeaderResponse {
    setHeader(name: string, value: string): void;
}

@ApiTags("training equipment increments")
@Controller({ path: "training/equipment-increments", version: "1" })
export class EquipmentIncrementController {
    constructor(
        @Inject(EQUIPMENT_INCREMENT_COMMANDS)
        private readonly commands: EquipmentIncrementCommands,
        @Inject(EQUIPMENT_INCREMENT_QUERIES)
        private readonly queries: EquipmentIncrementQueries,
        @Optional()
        @Inject(IDEMPOTENT_COMMAND_EXECUTOR)
        private readonly idempotency?: IdempotentCommandExecutor,
    ) {}

    @Get()
    @ApiOperation({ summary: "List configured equipment increments" })
    async list(): Promise<EquipmentIncrementListResponse> {
        return equipmentIncrementListResponseSchema.parse({ items: await this.queries.list() });
    }

    @Get("resolve")
    @ApiOperation({ summary: "Resolve the most specific increment for an exercise" })
    @ApiQuery({ name: "exerciseId", required: true, format: "uuid" })
    async resolve(@Query("exerciseId") exerciseId: string | undefined): Promise<EquipmentIncrementResponse> {
        const resolved = await this.queries.resolveForExercise(uuid(exerciseId, "exerciseId"));
        if (!resolved)
            throw new HttpException({ code: "NOT_FOUND", message: "No increment applies to that exercise" }, 404);
        return equipmentIncrementResponseSchema.parse(resolved);
    }

    @Post()
    @ApiOperation({ summary: "Create an equipment increment for the active profile" })
    @ApiHeader({ name: "Idempotency-Key", required: false })
    create(
        @Body() rawBody: unknown,
        @Headers("x-correlation-id") rawCorrelationId: string | undefined,
        @Headers("idempotency-key") idempotencyKey: string | undefined,
        @Res({ passthrough: true }) response: HeaderResponse,
    ): Promise<EquipmentIncrementResponse> {
        const request = parseContract(
            createEquipmentIncrementRequestSchema,
            rawBody,
            "Equipment increment validation failed",
        );
        const metadata = mutationMetadata(rawCorrelationId);
        return this.executeMutation({
            operation: "training.equipment-increment.create",
            idempotencyKey,
            request,
            metadata,
            response,
            status: 201,
            command: transaction => this.commands.create(request, metadata, transaction),
        });
    }

    @Get(":id")
    @ApiOperation({ summary: "Get one equipment increment" })
    @ApiParam({ name: "id", format: "uuid" })
    async get(
        @Param("id") id: string,
        @Res({ passthrough: true }) response: HeaderResponse,
    ): Promise<EquipmentIncrementResponse> {
        const resource = await this.queriesRead(id);
        if (!resource) throw new EquipmentIncrementNotFoundError(id);
        response.setHeader("ETag", formatRevisionEtag(resource.version));
        return equipmentIncrementResponseSchema.parse(resource);
    }

    @Patch(":id")
    @ApiOperation({ summary: "Update an equipment increment" })
    @ApiParam({ name: "id", format: "uuid" })
    @ApiHeader({ name: "If-Match", required: true })
    @ApiHeader({ name: "Idempotency-Key", required: false })
    update(
        @Param("id") id: string,
        @Body() rawBody: unknown,
        @Headers("if-match") ifMatch: string | undefined,
        @Headers("x-correlation-id") rawCorrelationId: string | undefined,
        @Headers("idempotency-key") idempotencyKey: string | undefined,
        @Res({ passthrough: true }) response: HeaderResponse,
    ): Promise<EquipmentIncrementResponse> {
        const request = parseContract(
            updateEquipmentIncrementRequestSchema,
            rawBody,
            "Equipment increment update validation failed",
        );
        const expectedVersion = expectedVersionFrom(ifMatch);
        const metadata = mutationMetadata(rawCorrelationId);
        return this.executeMutation({
            operation: "training.equipment-increment.update",
            idempotencyKey,
            request: { id, expectedVersion, body: request },
            metadata,
            response,
            status: 200,
            command: transaction => this.commands.update(id, expectedVersion, request, metadata, transaction),
        });
    }

    private async queriesRead(id: string): Promise<EquipmentIncrementResource | null> {
        const items = await this.queries.list();
        return items.find(item => item.id === id) ?? null;
    }

    private async executeMutation(input: {
        readonly operation: string;
        readonly idempotencyKey?: string;
        readonly request: unknown;
        readonly metadata: EquipmentIncrementMutationMetadata;
        readonly response: HeaderResponse;
        readonly status: number;
        readonly command: (transaction?: unknown) => Promise<EquipmentIncrementResource>;
    }): Promise<EquipmentIncrementResponse> {
        const perform = async (transaction?: unknown) =>
            equipmentIncrementResponseSchema.parse(await input.command(transaction));
        let body: EquipmentIncrementResponse;
        if (input.idempotencyKey !== undefined) {
            if (!this.idempotency) throw new Error("Idempotency support is not configured");
            const result = await this.idempotency.execute(
                {
                    operation: input.operation,
                    key: input.idempotencyKey,
                    request: input.request,
                    context: input.metadata,
                },
                async transaction => ({ status: input.status, body: await perform(transaction) }),
            );
            body = result.body;
            input.response.setHeader("Idempotency-Replayed", String(result.replayed));
        } else {
            body = await perform();
        }
        input.response.setHeader("ETag", formatRevisionEtag(body.version));
        return body;
    }
}

function uuid(value: string | undefined, field: string): string {
    try {
        return entityId(value ?? "");
    } catch {
        throw new ApplicationValidationError(`${field} must be a UUID`, { [field]: [`${field} must be a UUID`] });
    }
}

function parseContract<Output>(
    schema: {
        safeParse(
            value: unknown,
        ):
            | { success: true; data: Output }
            | { success: false; error: { issues: readonly { path: readonly PropertyKey[]; message: string }[] } };
    },
    value: unknown,
    message: string,
): Output {
    const parsed = schema.safeParse(value);
    if (!parsed.success) throw contractValidationException(message, parsed.error.issues);
    return parsed.data;
}

function contractValidationException(
    message: string,
    issues: readonly { path: readonly PropertyKey[]; message: string }[],
): HttpException {
    const fieldErrors: Record<string, string[]> = {};
    for (const issue of issues) {
        const path = issue.path.length > 0 ? issue.path.map(String).join(".") : "$";
        (fieldErrors[path] ??= []).push(issue.message);
    }
    return new HttpException({ code: "VALIDATION_FAILED", message, fieldErrors }, 422);
}

function mutationMetadata(rawCorrelationId: string | undefined): EquipmentIncrementMutationMetadata {
    return { correlationId: rawCorrelationId?.trim() || randomUUID(), actorId: null, source: "user" };
}

function expectedVersionFrom(ifMatch: string | undefined): number {
    if (!ifMatch) throw new ExpectedVersionRequiredError();
    try {
        return parseRevisionEtag(ifMatch);
    } catch (error) {
        throw new ApplicationValidationError(error instanceof Error ? error.message : "If-Match is invalid", {
            ifMatch: ["If-Match must be a quoted positive version"],
        });
    }
}
